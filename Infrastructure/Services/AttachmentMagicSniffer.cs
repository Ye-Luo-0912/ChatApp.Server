...
public static class AttachmentMagicSniffer
{
    ...
    /// <summary>
    /// Caller-owned small-buffer path. No temporary byte-array allocation is
    /// performed; only the recognized MIME string is returned.
    /// </summary>
    public static async Task<(string? ContentType, int BytesRead)> SniffAsync(
        Stream content,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var read = await ReadHeaderAsync(content, headerBuffer, cancellationToken)
            .ConfigureAwait(false);
        return (Sniff(headerBuffer.Span[..read]), read);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var n = await content.ReadAsync(buffer[read..], cancellationToken)
                .ConfigureAwait(false);
            if (n == 0)
                break;
            read += n;
